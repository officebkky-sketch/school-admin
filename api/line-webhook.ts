...
async function handleFullAIQuery(replyToken: string, message: string, profile: any) {
  try {
    ...
    [ข้อมูลเสริมจากคลังเอกสาร]
    ${knowledgeContext || "ไม่พบข้อมูลเสริม"}

    ผู้ถาม: คุณครู ${profile.display_name} (สิทธิ์: ${profile.role})
    คำถาม: ${message}

    คำแนะนำในการตอบ (STRICT RULES):
    1. ตอบเฉพาะเนื้อหาที่จะแสดงผลบน LINE เท่านั้น ห้ามมีคำเกริ่นนำเรื่อง Persona หรือเงื่อนไขใดๆ ทั้งสิ้น
    2. จัดรูปแบบให้สวยงามระดับพรีเมียม:
       - ใช้ Emoji นำหน้าหัวข้อสำคัญเสมอ (📝, 📊, 📋, 💡, ✅)
       - ใช้ **ตัวหนา** เน้นประเด็นสำคัญ ยอดเงิน หรือชื่อคน
       - เว้นบรรทัดให้โปร่งตา อ่านง่ายบนมือถือ
       - ห้ามใช้ตารางแบบ Markdown ให้ใช้รายการสัญลักษณ์ (Bullet points) แทน
    
    ตัวอย่างรูปแบบที่ต้องการ:
    📝 **รายงานรายชื่อนักเรียน ชั้น อ.3**
    📅 **ปีการศึกษา:** 2569
    🏫 **ห้องเรียน:** 1
    👥 **จำนวนทั้งหมด:** 9 คน

    📋 **รายชื่อนักเรียน:**
    1️⃣ **3870** เด็กชายอัชรอฟ น่วมหมุน
    2️⃣ **3871** เด็กหญิงอริสา ทิพย์จำนงค์
    ...
    💡 หากต้องการข้อมูลอื่นเพิ่มเติม แจ้งผมได้เลยนะครับ!

    ตอบอย่างเป็นมืออาชีพ สุภาพ มีหางเสียง (ครับ/ค่ะ) และสร้างพลังบวก`;
